...
async def test_delete_task_success(
    client, create_task_in_database, get_task_from_database
):
    # Create a new task in the database to delete
    task_data = {"id": 1, "user_id": 123321, "task": "Помыть посуду"}
...
async def test_get_all_tasks(client, create_task_in_database):
    ...
    task_data = {"id": 1, "user_id": 123321, "task": "Помыть посуду"}
    task_data_2 = {"id": 2, "user_id": 123321, "task": "Помыть кошку"}
    await create_task_in_database(**task_data)
...
async def test_add_tag(client, async_session_test):
    ...
    response = client.put(
        f"task/add_tag?task_id={task.id}&tag_name={tag.tag}&user_id={task.user_id}"
    ).json()
...
async def test_create_tag(client, async_session_test):
    task_data = {"tag": "test tag", "user_id": 133}
...
async def test_get_all_tags(client, async_session_test):
    tag_data = {"tag": "test tag", "user_id": 133}
    tag_data_2 = {"tag": "test tag 2", "user_id": 133}
    async with async_session_test() as session:
        async with session.begin():
            tag_1 = Tag(**tag_data)
