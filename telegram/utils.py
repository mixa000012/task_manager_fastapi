...
async def delete_category(user_id: int, category_name: str):
    requests.delete(f"{task_url}/delete_tag?user_id={user_id}&tag={category_name}")


async def get_all_tasks(
    chat_id, bot, message_id=None, user_id=None, chat_type=None, tag=None
):
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    text=task.get("title"),
                    callback_data=f"delete_task:{task.get('id')}:{tag}",
                )
            ]
            for task in tasks
        ]
